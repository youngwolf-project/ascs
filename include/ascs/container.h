...
//ascs requires that container must take one and only one template argument.
	...
	(!defined(__CYGWIN__) && !defined(__MINGW32__) && !defined(__MINGW64__) && __GNUC__ >= 5) //terrible cygwin and mingw
template<typename T> using list = std::list<T>;
//on cygwin and mingw, even gcc 7 still has not made list::size() to be O(1) complexity, which also means function size() and empty() are not thread safe,
//but ascs::queue needs them to be thread safe no matter itself is lockable or dummy lockable (see ascs::queue for more details),
//so, we must use ascs::list instead of std::list for cygwin and mingw.
#else
