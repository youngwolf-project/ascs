 ...
 *
 * make ascs support ssl (based on boost::asio::ssl)
 */
...
{
public:
	template<typename Arg> socket(Arg&& arg, boost::asio::ssl::context& ctx_) : Socket(std::forward<Arg>(arg), ctx_), ctx(ctx_) {}
...
protected:
	...
	virtual void on_recv_error(const boost::system::error_code& ec) {this->stop_graceful_shutdown_monitoring(); Socket::on_recv_error(ec);}
...
public:
	client_socket_base(service_pump& service_pump_, boost::asio::ssl::context& ctx_) : super(service_pump_, ctx_) {}
...
private:
	virtual void connect_handler(const boost::system::error_code& ec) //intercept tcp::client_socket_base::connect_handler
	{
		if (!ec)
		{
			...
			this->next_layer().async_handshake(boost::asio::ssl::stream_base::client, this->make_handler_error([this](const boost::system::error_code& ec) {handle_handshake(ec);}));
		}
		else
...
protected:
	...
	{
		this->status = super::link_status::HANDSHAKING;
		this->next_layer().async_handshake(boost::asio::ssl::stream_base::server, this->make_handler_error([this](const boost::system::error_code& ec) {handle_handshake(ec);}));
		return true;
	}
...
private:
	std::shared_ptr<boost::asio::ssl::context> ctx_holder;
};
