 ...
 *
 * make ascs support websocket (based on boost::beast)
 */

#ifndef _ASCS_WEBSOCKET_H_
#define _ASCS_WEBSOCKET_H_
...
namespace ascs { namespace websocket {
...
protected:
	...
	virtual void connect_handler(const boost::system::error_code& ec) //intercept tcp::client_socket_base::connect_handler
	{
		...
		this->next_layer().set_option(boost::beast::websocket::stream_base::decorator([](boost::beast::websocket::request_type& req) {
			req.set(boost::beast::http::field::user_agent, "ascs websocket client based on " BOOST_BEAST_VERSION_STRING);
		}));
#endif
		// Provide the value of the Host HTTP header during the WebSocket handshake.
		// See https://tools.ietf.org/html/rfc7230#section-5.4
		// Perform the websocket handshake
		this->next_layer().async_handshake(this->endpoint_to_string(this->get_server_addr()), "/", this->make_handler_error([this](const asio::error_code& ec) {handle_handshake(ec);}));
	}
...
protected:
	virtual bool do_start() //intercept tcp::server_socket_base::do_start (to add handshake)
	{
		...
		this->next_layer().set_option(boost::beast::websocket::stream_base::decorator([](boost::beast::websocket::response_type& res) {
			res.set(boost::beast::http::field::server, "ascs websocket server based on " BOOST_BEAST_VERSION_STRING);
		}));
#endif
		// Accept the websocket handshake
		this->next_layer().async_accept(this->make_handler_error([this](const asio::error_code& ec) {handle_handshake(ec);}));
		return true;
	}
