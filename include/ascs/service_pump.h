...
public:
	...
	service_pump(int concurrency_hint = BOOST_ASIO_CONCURRENCY_HINT_SAFE) : started(false), first(true), real_thread_num(0), del_thread_num(0), single_ctx(true), single_thread(false)
	...
	service_pump(int concurrency_hint = BOOST_ASIO_CONCURRENCY_HINT_SAFE, bool multi_ctx = false) : started(false), first(true), single_ctx(!multi_ctx), single_thread(false)
		{context_can.emplace_back(concurrency_hint);}
...
#else
#ifdef ASCS_DECREASE_THREAD_AT_RUNTIME
	service_pump() : started(false), first(true), real_thread_num(0), del_thread_num(0), single_ctx(true), single_thread(false), context_can(1) {}
#else
	//basically, the parameter multi_ctx is designed to be used by single_service_pump, which means single_service_pump always think it's using multiple io_context
	//for service_pump, you should use set_io_context_num function instead if you really need multiple io_context.
	service_pump(bool multi_ctx = false) : started(false), first(true), single_ctx(!multi_ctx), single_thread(false), context_can(1) {}
...
protected:
	//not thread safe
#if BOOST_ASIO_VERSION >= 101200
	void do_add_service_thread(bool first, int thread_num, bool block = false, int io_context_num = 0, int concurrency_hint = BOOST_ASIO_CONCURRENCY_HINT_SAFE)
#else
