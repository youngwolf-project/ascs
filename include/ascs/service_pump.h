...
public:
	...
	service_pump(int concurrency_hint = ASIO_CONCURRENCY_HINT_SAFE) : single_ctx(true) {context_can.emplace_back(concurrency_hint);}
#else
	...
	service_pump(int concurrency_hint = ASIO_CONCURRENCY_HINT_SAFE, bool multi_ctx = false) : single_ctx(!multi_ctx)
