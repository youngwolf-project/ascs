...
public:
	...
	void recv_msg() {if (!reading && is_ready()) dispatch_strand(rw_strand, [this]() {do_recv_msg();});}
#else
private:
	void recv_msg() {dispatch_strand(rw_strand, [this]() {do_recv_msg();});}
public:
#endif
...
private:
	...
	void send_msg() {dispatch_strand(rw_strand, [this]() {do_send_msg();});}
#else
	void send_msg() {if (!sending && is_ready()) dispatch_strand(rw_strand, [this]() {do_send_msg();});}
#endif
...
protected:
	...
	bool close(bool use_close = false) //if not use_close, shutdown (both direction) will be used
	{
		...
		else
		{
			...
			set_timer(TIMER_DELAY_CLOSE, ASCS_DELAY_CLOSE * 1000 + 50, [this](tid id)->bool {return timer_handler(TIMER_DELAY_CLOSE);});
		}
...
private:
	...
	void dispatch_msg() {if (!dispatching) post_strand(dis_strand, [this]() {do_dispatch_msg();});}
...
#endif
				set_timer(TIMER_DISPATCH_MSG, msg_handling_interval_, [this](tid id)->bool {return timer_handler(TIMER_DISPATCH_MSG);}); //hold dispatching
			}
			else
			{
#else
	...
	bool timer_handler(tid id)
	{
		...
		case TIMER_DISPATCH_MSG:
			post_strand(dis_strand, [this]() {do_dispatch_msg();});
			break;
