...
namespace tcp
{
	...
	public:
		virtual bool del_socket(const std::shared_ptr<tracked_executor>& socket_ptr, bool need_shutdown = true) = 0;
...
namespace udp
{
	...
	public:
		...
		udp_msg(const typename Family::endpoint& _peer_addr, MsgType&& msg) : MsgType(std::move(msg)), peer_addr(_peer_addr) {}
...
#define POP_FIRST_PENDING_MSG_NOTIFY(FUNNAME, CAN, MSGTYPE) void FUNNAME(MSGTYPE& msg) \
	{msg.clear(); if (CAN.try_dequeue(msg)) {if (msg.p) msg.p->set_value(sync_call_result::NOT_APPLICABLE);}}
...
{ \
	...
	auto re = this->packer()->pack_msg(std::move(msg), msg_can); \
	dur.end(); \
...
{ \
	...
	auto_duration dur(stat.pack_time_sum); \
	auto re = this->packer()->pack_msg(std::move(msg1), std::move(msg2), msg_can); \
	dur.end(); \
	return re && do_direct_send_msg(msg_can, prior); \
} \
...
{ \
	if (!can_overflow && !this->shrink_send_buffer()) \
		return false; \
	auto_duration dur(stat.pack_time_sum); \
	auto msg = this->packer()->pack_msg(pstr, len, num, NATIVE); \
	dur.end(); \
	return do_direct_send_msg(std::move(msg), prior); \
} \
...
{ \
	if (!can_overflow && !this->shrink_send_buffer()) \
		return false; \
	in_msg_type msg(peer_addr, this->packer()->pack_msg(pstr, len, num, NATIVE)); \
	return do_direct_send_msg(std::move(msg), prior); \
} \
